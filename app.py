...
def modificar_servicio(vmid):
    ...
    if servicio.nombre_servicio == 'dns':
        ...
        return render_template('modificar_web.html', vmid=vmid, ip=registro.ip_asignada)
    elif servicio.nombre_servicio == 'dhcp':
        return render_template('modificar_dhcp.html', vmid=vmid, ip=registro.ip_asignada)
    else:
...
def modificar_dns():
    ...
    try:
        ...
        if not registro:
            flash("Acceso denegado al servidor DNS.", "danger")
            return redirect(url_for('gestion_servicio'))
            
        orquestador = SSHManager(ip=registro.ip_asignada, password=os.getenv("SSH_PASS"))
        exito, resultado = orquestador.aniadir_registro_dns(zona_dominio, nombre_host, tipo_registro, valor_destino, ttl)
        
        if exito:
            flash(f"Registro DNS añadido correctamente en el servidor {vmid}.", "success")
        else:
            flash(f"Error al configurar DNS: {resultado}", "danger")
    except Exception as e:
...
def modificar_web():
    ...
    try:
        ...
        if exito:
            ...
            flash(f"Error al actualizar la web: {resultado}", "danger")
...
def modificar_dhcp():
    ...
    try:
        ...
        if exito:
            ...
            flash(f"Error al modificar DHCP: {resultado}", "danger")
    except Exception as e:
